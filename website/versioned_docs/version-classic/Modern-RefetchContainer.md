...
* `options`: Optional object containing set of options.
  * `force`: If the [Network Layer](./classic-network-layer.html) has been configured with a cache, this option forces a refetch even if the data for this query and variables is already available in the cache.
