...
var RelayContext = (function () {
  ...
  RelayContext.prototype.reset = function reset() {
    !(!this._storeData.getChangeEmitter().hasActiveListeners() && !this._storeData.getMutationQueue().hasPendingMutations() && !this._storeData.getPendingQueryTracker().hasPendingQueries()) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayStore.reset(): Cannot reset the store while there are active ' + 'Relay Containers or pending mutations/queries.') : invariant(false) : undefined;
    this._storeData = new RelayStoreData();
  };
