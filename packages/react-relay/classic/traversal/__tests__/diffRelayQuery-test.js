...
describe('diffRelayQuery', () => {
  ...
  it('does not fetch known connection metadata for unfetched ranges', () => {
    ...
    const body = RelayClassic_DEPRECATED.QL`
      fragment on Comment {
        body {
          text
        }
  ...
  it('splits viewer-rooted queries', () => {
    ...
    expect(diffQueries[0].getName()).toBe(query.getName());
    expect(diffQueries[0]).toEqualQueryRoot(
      getNode(RelayClassic_DEPRECATED.QL`query{viewer{primaryEmail}}`),
    );
  });
  ...
  it('reuses fields and fragments if both unchanged', () => {
    const records = {};
    const store = new RelayRecordStore({records});
    const frag = RelayClassic_DEPRECATED.QL`fragment on Node {name}`;
    const query = getNode(
      RelayClassic_DEPRECATED.QL`
      query {
        node(id:"4") {
  ...
  it('handles missing fields in fragments', () => {
    ...
    const store = new RelayRecordStore({records});
    const firstNameFrag = RelayClassic_DEPRECATED.QL`
      fragment on Node {
        firstName
      }
    `;
    const lastNameFrag = RelayClassic_DEPRECATED.QL`
      fragment on Node {
        lastName
      }
    `;
    const nestingFrag = RelayClassic_DEPRECATED.QL`
      fragment on Node {
        ${firstNameFrag}
        ${lastNameFrag}
      }
    ...
    const expectedFragment = RelayClassic_DEPRECATED.QL`
      fragment on Node {
        ${firstNameFrag}
      }
    `;
    ...
    const query0Fragments = query0
      .getChildren()
      .filter(child => child instanceof RelayQuery.Fragment);
  ...
  it('fetches an extension of a range', () => {
    ...
    mockRange.retrieveRangeInfoForQuery.mockReturnValue({
      ...
      diffCalls: [{name: 'after', value: 'cursor1'}, {name: 'first', value: 4}],
    });
  ...
  it('does not fetch plural fields with null linked field children', () => {
    const records = {
      '12345': {
        __dataID__: '12345',
        id: '12345',
        allPhones: [{__dataID__: 'client:1'}, {__dataID__: 'client:2'}],
      },
      'client:1': {
        __dataID__: 'client:1',
        isVerified: false,
