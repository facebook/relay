...
impl Schema for SchemaWrapper {
    fn query_type(&self) -> Option<Type> {
        // TODO
        self.get_type("Query".intern())
    }

    fn mutation_type(&self) -> Option<Type> {
        self.get_type("Mutation".intern())
    }

    fn subscription_type(&self) -> Option<Type> {
        self.get_type("Subscription".intern())
    }
    ...
    fn field(&self, id: FieldID) -> &Field {
        self.fields.get(id, || match id {
            TYPENAME_FIELD_ID => Field {
                ...
                type_: TypeReference::NonNull(Box::new(TypeReference::Named(
                    self.get_type("String".intern()).unwrap(),
                ))),
                directives: Vec::new(),
                parent_type: None,
            },
            CLIENTID_FIELD_ID => Field {
                name: self.clientid_field_name,
                is_extension: true,
                arguments: ArgumentDefinitions::new(Default::default()),
                type_: TypeReference::NonNull(Box::new(TypeReference::Named(
                    self.get_type("ID".intern()).unwrap(),
                ))),
                directives: Vec::new(),
                parent_type: None,
            },
            FETCH_TOKEN_FIELD_ID => Field {
