...
function getFragmentVariables(
  ...
  fragment.argumentDefinitions.forEach(definition => {
    switch (definition.kind) {
      case 'LocalArgument':
        const argumentWasPassed =
          argumentVariables.hasOwnProperty(definition.name) &&
          argumentVariables[definition.name] !== undefined;
        if (argumentWasPassed) {
          variables[definition.name] = argumentVariables[definition.name];
          break;
        }
