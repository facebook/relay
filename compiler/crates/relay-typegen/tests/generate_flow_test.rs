...
#[test]
fn typename_on_node() {
  let input = include_str!("generate_flow/fixtures/typename-on-node.graphql");
  let expected = include_str!("generate_flow/fixtures/typename-on-node.expected");
  test_fixture(transform_fixture, "typename-on-node.graphql", "generate_flow/fixtures/typename-on-node.expected", input, expected);
}
