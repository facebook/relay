...
use graphql_ir::{
    Condition, Field, FragmentDefinition, FragmentSpread, InlineFragment, LinkedField, Program,
    Selection, Validator,
};
...
impl<'a> UpdatableFragmentSpread<'a> {
    ...
    fn ensure_discriminated_union_is_created(
        &self,
        ...
        if !type_reference.inner().is_abstract_type() {
            errors.push(Diagnostic::error(
                ValidationMessage::UpdatableFragmentSpreadContainedInConcreteLinkedField {
                    linked_field_type: self.program.schema.get_type_name(type_reference.inner()),
        ...
        for selection in linked_field.selections.iter() {
            match selection {
                Selection::InlineFragment(inline_fragment) => {
                    if let Err(e) = self.ensure_inline_fragment_is_valid(
                        inline_fragment,
                        linked_field.definition.location,
                ...
                _ => {
                    errors.push(Diagnostic::error(ValidationMessage::UpdatableFragmentSpreadLinkedFieldContainsNonInlineFragment {
                        linked_field_name_or_alias: linked_field.alias_or_name(&self.program.schema)
                    }, linked_field.definition.location))
                }
            }
        }
                ...
                if encountered_type_conditions.contains(&type_condition) {
                    errors.push(Diagnostic::error(
                        ValidationMessage::UpdatableFragmentSpreadLinkedFieldContainsInlineFragmentClashingRefinement {
                            concrete_type: self.program.schema.get_type_name(type_condition),
                        },
                        linked_field_location
                    ))
                }
...
impl<'a> Validator for UpdatableFragmentSpread<'a> {
    ...
    fn validate_linked_field(&mut self, linked_field: &LinkedField) -> DiagnosticsResult<()> {
        ...
        if linked_field_item.should_ensure_discriminated_union_is_created {
            match self.ensure_discriminated_union_is_created(linked_field) {
                Ok(_) => {}
