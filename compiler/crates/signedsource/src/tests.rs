...
#[test]
fn test_sign_file() {
    let new_signing_result_1 = sign_file(&format!("# {SIGNING_TOKEN}\ntest 1"));
    assert_eq!(
        new_signing_result_1,
        "# \x40generated SignedSource<<d9b7b52f54978f54b84a0fd48145e470>>\ntest 1"
    );

    let new_signing_result_2 = sign_file(&format!("# {SIGNING_TOKEN}\ntest 2"));
    assert_eq!(
...
fn test_is_signed() {
    ...
    let mut signed_string = sign_file(&format!("# {SIGNING_TOKEN}\ntest"));
