...
describe('ReactRelayQueryRenderer', () => {
  ...
  describe('when new props are received', () => {
    ...
    it('does not update if all props are ===', () => {
      ...
      expect(environment.sendQuerySubscription).not.toBeCalled();
  ...
  describe('when the fetch succeeds', () => {
    ...
    it('subscribes to the root fragment', () => {
      ...
      expect(environment.subscribe.mock.calls[0][0].node).toBe(TestQuery.fragment);
      expect(environment.subscribe.mock.calls[0][0].variables).toEqual(variables);
    });
  });
  ...
  describe('when props change during a fetch', () => {
    ...
    it('cancels the pending fetch', () => {
      const disposeFetch = environment.sendQuerySubscription.mock.dispose;
