...
class GraphQLDeferredQueryTracker {
  ...
  recordQuery(
    ...
    if (parentID) {
      ...
      if (deferredFragmentNames) {
        ...
        var dataIDs = getRootCallToIDMap(query, this._recordStore);
  ...
  _resolveDeferredQuery(
    ...
    if (batchCall) {
      ...
      dataIDs = getRootCallToIDMap(query, this._recordStore);
    }
  ...
  _resolveFragmentsForRootCall(
    ...
    var rootCallMap = getRootCallToIDMap(query, this._recordStore);
  ...
  _broadcastChangeForFragment(
    ...
    if (!this._broadcastItems) {
      this._broadcastItems = [];
      resolveImmediate(() => this._processBroadcasts());
    }
    // $FlowFixMe: We don't have to worry about this being invalid
    this._broadcastItems.push({dataID, fragmentID, error: null});
  }
  ...
  _broadcastErrorForFragment(
    ...
    // $FlowFixMe: We don't have to worry about this being invalid
    this._broadcastItems.push({dataID, fragmentID, error});
  }
