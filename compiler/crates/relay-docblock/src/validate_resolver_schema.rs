...
fn validate_strong_resolver_types(schema: &SDLSchema) -> DiagnosticsResult<()> {
    try_all(
        schema
            ...
            .map(|weak_model_object| {
                validate_implements_interfaces_against_schema(weak_model_object, schema)
            }),
    )?;
    Ok(())
}
...
fn is_valid_mutation_resolver_return_type(schema: &SDLSchema, type_: &TypeReference<Type>) -> bool {
    match type_ {
        ...
        TypeReference::NonNull(non_null_type) => {
            ...
            return is_valid_mutation_resolver_return_type(schema, &non_null_type.as_ref());
        }
    }
}
...
/// should disallow it.
fn validate_implements_interfaces_against_schema(
    object: &Object,
    schema: &SDLSchema,
) -> DiagnosticsResult<()> {
