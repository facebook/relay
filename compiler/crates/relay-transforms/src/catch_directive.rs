...
use self::catchable_field::Catchable;
...
impl<'program> CatchDirective<'program> {
    ...
    fn get_catch_metadata<T: Catchable>(&mut self, field: &T) -> Option<CatchMetadata> {
    ...
    fn assert_not_with_required<T: Catchable>(&mut self, field: &T) {
