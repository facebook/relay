...
- ensure that `[JSModule].relayprovider.js` exists and exports a `get()`
  ...
  - if the value can legitimately differ between runs (for example, it depends
    on per-request or per-session state), also export `dynamic: true`. Relay
    then resolves the provider fresh on every operation instead of memoizing the
    first value, which is important when one runtime serves many requests, such
    as during server-side rendering. `get` should still return a consistent
...
A `dynamic` provider whose value depends on the current run:

```javascript
// Viewer_IsLoggedIn.relayprovider.js
export default {
  // resolved fresh on every operation rather than memoized
  dynamic: true,
  get(): boolean {
