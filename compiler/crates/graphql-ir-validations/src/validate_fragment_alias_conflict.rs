...
struct SelectionContext {
    seen_fields: DashMap<StringKey, Location>,
    seen_fragment_aliases: DashMap<StringKey, Location>,
}
...
impl<'s> ValidateFragmentAliasConflict<'s> {
        ...
        for<'b> F: FnOnce(&'b mut Self) -> DiagnosticsResult<T>,
    {
        ...
        let result = f(self)?;
        for entry in self.selection_context.seen_fragment_aliases.iter() {
            if let Some(field_location) = self.selection_context.seen_fields.get(entry.key()) {
                return Err(vec![
                    Diagnostic::error(
                        ValidationMessage::FragmentAliasConflictsWithField {
                            response_key: *entry.key(),
                        },
                        *entry.value(),
                    )
