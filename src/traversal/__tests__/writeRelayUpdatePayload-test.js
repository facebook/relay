...
describe('writePayload()', () => {
  ...
  describe('range delete mutations', () => {
    ...
    it('optimistically removes range edges', () => {
      ...
      expect(queueStore.getRecordStatus(edgeID)).toBe('NONEXISTENT');
      expect(queueStore.getRecordStatus(commentID)).toBe('EXISTENT');
      ...
      expect(store.getRecordStatus(edgeID)).toBe('EXISTENT');
    ...
    it('non-optimistically removes range edges', () => {
      ...
      expect(store.getRecordStatus(edgeID)).toBe('NONEXISTENT');
      expect(store.getRecordStatus(commentID)).toBe('EXISTENT');
      // the range no longer returns this edge
      expect(store.getRangeMetadata(
        connectionID,
        [{name: 'first', value: '1'}]
  ...
  describe('node/range delete mutations', () => {
    ...
    it('optimistically deletes comments', () => {
      ...
      // node is deleted
      expect(queueStore.getRecordStatus(firstCommentID)).toBe('NONEXISTENT');
      expect(queueStore.getRecordStatus(secondCommentID)).toBe('EXISTENT');
      // corresponding edge is deleted for every range this node appears in
      expect(queueStore.getRecordStatus(firstEdgeID)).toBe('NONEXISTENT');
      expect(queueStore.getRecordStatus(secondEdgeID)).toBe('EXISTENT');
      // the range no longer returns this edge
      expect(queueStore.getRangeMetadata(
        connectionID,
        [{name: 'first', value: '2'}]
      ...
      expect(store.getRecordStatus(firstCommentID)).toBe('EXISTENT');
      expect(store.getRecordStatus(secondCommentID)).toBe('EXISTENT');
      expect(store.getRecordStatus(firstEdgeID)).toBe('EXISTENT');
      expect(store.getRecordStatus(secondEdgeID)).toBe('EXISTENT');
      expect(store.getField(connectionID, 'count')).toBe(1);
      expect(store.getRangeMetadata(
        connectionID,
        [{name: 'first', value: '2'}]
    ...
    it('non-optimistically deletes comments', () => {
      ...
      // node is deleted
      expect(store.getRecordStatus(firstCommentID)).toBe('NONEXISTENT');
      expect(store.getRecordStatus(secondCommentID)).toBe('EXISTENT');
      // corresponding edge is deleted for every range this node appears in
      expect(store.getRecordStatus(firstEdgeID)).toBe('NONEXISTENT');
      expect(store.getRecordStatus(secondEdgeID)).toBe('EXISTENT');
      // the range no longer returns this edge
      expect(store.getRangeMetadata(
        connectionID,
  ...
  describe('plural node delete mutation', () => {
    ...
    it('optimistically deletes requests', () => {
      ...
      // node is deleted
      expect(queueStore.getRecordStatus(firstRequestID)).toBe('NONEXISTENT');
      expect(queueStore.getRecordStatus(secondRequestID)).toBe('NONEXISTENT');
      // third node is not deleted
      expect(queueStore.getRecordStatus(thirdRequestID)).toBe('EXISTENT');

      // base records are not modified: node & edge exist, the edge is still
      // in the range, and the connection metadata is unchanged
      expect(store.getRecordStatus(firstRequestID)).toBe('EXISTENT');
      expect(store.getRecordStatus(secondRequestID)).toBe('EXISTENT');
      expect(store.getRecordStatus(thirdRequestID)).toBe('EXISTENT');
    });
    ...
    it('non-optimistically deletes requests', () => {
      ...
      // node is deleted
      expect(store.getRecordStatus(firstRequestID)).toBe('NONEXISTENT');
      expect(store.getRecordStatus(secondRequestID)).toBe('NONEXISTENT');
      // third node is not deleted
      expect(store.getRecordStatus(thirdRequestID)).toBe('EXISTENT');
    });
  });
  ...
  describe('range add mutations', () => {
    ...
    it('optimistically prepends comments', () => {
      ...
      expect(store.getField(connectionID, 'count')).toBe(1);
      expect(store.getRecordStatus(nextEdgeID)).toBe('UNKNOWN');
      expect(store.getRecordStatus(nextNodeID)).toBe('UNKNOWN');
      expect(store.getRecordStatus(bodyID)).toBe('UNKNOWN');
      expect(store.getRangeMetadata(
        connectionID,
        [{name: 'first', value: '2'}]
      ).requestedEdges.map(edge => edge.edgeID)).toEqual([
