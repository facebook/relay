...
var RelayQLType = function () {
  ...
  _createClass(RelayQLType, [{
    ...
    key: 'getNodeIdField',
    value: function getNodeIdField() {
      var _this17 = this;

      var node = this.context.schema.getType('Node');
      if (node && node instanceof types.GraphQLInterfaceType) {
        var _ret2 = function () {
          var fields = node.getFields();
          var field = find(Object.keys(fields), function (fieldName) {
            var type = fields[fieldName].type.ofType;
            return type && type.name === 'ID';
          });

          if (field && _this17.hasField(field)) {
            var nodeInterface = _this17.schemaUnmodifiedType === node && _this17;
            if (!nodeInterface) {
              nodeInterface = find(_this17.getInterfaces(), function (iface) {
                return iface.schemaUnmodifiedType === node;
              });
            }
            if (!nodeInterface) {
              console.warn('Using ' + field + ' of ' + _this17.schemaUnmodifiedType.name + ' ' + 'as Node ID, even though it does not explicitly ' + 'implement the Node interface.');
            }
            return {
              v: field
            };
    ...
    key: 'getIdField',
    value: function getIdField(defaultField) {
