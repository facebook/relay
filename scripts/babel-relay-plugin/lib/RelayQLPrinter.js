    ...
    _createClass(RelayQLPrinter, [{
      ...
      value: function printQuery(query) {
        ...
        if (identifyingArgs.length) {
          invariant(identifyingArgs.length === 1, 'Queries may have at most one identifying argument - ' + '(type ID or [ID]), but field `%s` has %s. Use `Int` or `String` ' + 'instead.', rootField.getName(), identifyingArgs.length);
