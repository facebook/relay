...
function runWithFeatureFlags(setFlags: (typeof RelayFeatureFlags) => void) {
  describe('execute() a query with nested @match', () => {
    ...
    it('resolves sibling modules in one tick, and notifies once if the feature flag is on', () => {
      ...
      const outerRendererASnapshot = environment.lookup(outerRendererASelector);
      ...
      const outerRendererBSnapshot = environment.lookup(outerRendererBSelector);
      ...
      if (RelayFeatureFlags.BATCH_ASYNC_MODULE_UPDATES_FN != null) {
        ...
        nextOuterRendererASnapshot = outerRendererACallback.mock.calls[1][0];
      }
    ...
    it('resolves sibling and nested modules in one tick, and notifies once if the feature flag is on', () => {
      ...
      if (RelayFeatureFlags.BATCH_ASYNC_MODULE_UPDATES_FN != null) {
        ...
        expect(outerRendererBCallback).toBeCalledTimes(1);
        nextOuterRendererASnapshot = outerRendererACallback.mock.calls[0][0];
        nextOuterRendererBSnapshot = outerRendererBCallback.mock.calls[0][0];
      } else {
        expect(outerRendererACallback).toBeCalledTimes(3);
        expect(outerRendererBCallback).toBeCalledTimes(2);
        nextOuterRendererASnapshot = outerRendererACallback.mock.calls[2][0];
        nextOuterRendererBSnapshot = outerRendererBCallback.mock.calls[1][0];
      ...
      const innerSelector = nullthrows(
        getSingularSelector(
          plaintextRendererFragment,
          (nextOuterRendererASnapshot.data?.user: $FlowFixMe)?.innerRenderer,
        ),
      );
