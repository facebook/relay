...
describe('recycleNodesInto', () => {
  ...
  describe('objects', () => {
    test('recycles empty objects', () => {
      const prevData = {};
      const nextData = {};
      expect(recycleNodesInto(prevData, nextData)).toBe(prevData);
    });

    test('recycles nested empty objects', () => {
      const prevData = {foo: {bar: {}}};
      const nextData = {foo: {bar: {}}};
      expect(recycleNodesInto(prevData, nextData)).toBe(prevData);
    });

    test('recycles equal leaf objects', () => {
      const prevData = {foo: 1};
      const nextData = {foo: 1};
      expect(recycleNodesInto(prevData, nextData)).toBe(prevData);
    });

    test('recycles parent objects with equal leaf objects', () => {
      const prevData = {foo: {bar: 1}};
      const nextData = {foo: {bar: 1}};
      expect(recycleNodesInto(prevData, nextData)).toBe(prevData);
    });

    test('does not recycle unequal leaf objects', () => {
      const prevData = {foo: 1};
      const nextData = {foo: 100};
      expect(recycleNodesInto(prevData, nextData)).not.toBe(prevData);
    });

    test('does not recycle parent objects with unequal leaf objects', () => {
      const prevData = {foo: {bar: 1}};
      const nextData = {foo: {bar: 100}};
      expect(recycleNodesInto(prevData, nextData)).not.toBe(prevData);
    });

    test('does not recycle object with fewer properties', () => {
      const prevData = {foo: 1};
    ...
    test('recycles equal leaf objects with unequal parent objects', () => {
    ...
    test('recycles identical objects', () => {
      const prevData = {foo: {bar: 1}, baz: 2};
    ...
    test('recycles identical leaves', () => {
      const prevData = {foo: 1};
      const nextData = {foo: 1};
      // "next" data should not be modified if it is === to previous data
    ...
    test('does not mutate frozen equal parent objects with equal leaf objects', () => {
      const prevData = {foo: {bar: 1}};
      const nextData = {foo: {bar: 1}};
      Object.freeze(nextData);
    ...
    test('does not mutate frozen unequal parent objects with equal leaf objects', () => {
      const prevData = {foo: {bar: 1}, baz: 2};
      const nextData = {foo: {bar: 1}, baz: 200};
      Object.freeze(nextData);
      Object.freeze(nextData.foo);
    ...
    test('does not recycle arrays as objects', () => {
      const prevData = [1, 2];
  ...
  describe('arrays', () => {
    test('recycles arrays with equal scalars', () => {
    ...
    test('does not recycle arrays with unequal scalars', () => {
      const prevData = [1, 2, 3];
    ...
    test('recycles arrays with equal objects without mutating `prevData`', () => {
      const prevData = [{foo: 1}, {bar: 2}];
      const nextData = [{foo: 1}, {bar: 2}];
      expect(recycleNodesInto(prevData, nextData)).toBe(prevData);
    });

    test('recycles arrays with equal objects without mutating frozen `nextData`', () => {
      const prevData = [{foo: 1}, {bar: 2}];
      const nextData = [{foo: 1}, {bar: 2}];
      Object.freeze(nextData);
      expect(recycleNodesInto(prevData, nextData)).toBe(prevData);
    ...
    test('recycles arrays without mutating `prevData`', () => {
    ...
    test('recycles arrays with equal objects but unequal parent objects', () => {
    ...
    test('recycles equal objects from an array with unequal siblings', () => {
      const prevData = [{foo: 1}, {bar: 2}];
    ...
    test('recycles equal objects from an array with fewer siblings', () => {
      const prevData = [{foo: 1}];
      const nextData = [{foo: 1}, {bar: 2}];
      const recycled = recycleNodesInto(prevData, nextData);
    ...
    test('recycles equal objects from an array with more siblings', () => {
      const prevData = [{foo: 1}, {bar: 2}];
      const nextData = [{foo: 1}];
      const recycled = recycleNodesInto(prevData, nextData);
    ...
    test('does not recycle objects as arrays', () => {
  ...
  describe('weaksets', () => {
    describe('when not supported by environment', () => {
      ...
      test('handles weaksets being undefined', () => {
        const data = {};
        expect(() => recycleNodesInto(data, data)).not.toThrow();
      });
    });

    test('does not recycle weaksets with unequal values', () => {
      const prevData = new WeakSet([{a: 1}, {b: 2}, {c: 3}]);
      const nextData = new WeakSet([{a: 4}, {b: 5}, {c: 6}]);
      expect(recycleNodesInto(prevData, nextData)).toBe(nextData);
    });

    test('does not recycle weaksets with equal values', () => {
      const prevData = new WeakSet([{a: 1}, {b: 2}, {c: 3}]);
  ...
  describe('maps', () => {
    test('does not recycle maps with unequal values', () => {
    ...
    test('does not recycle maps with equal values', () => {
      const prevData = new Map();
      prevData.set('a', 1);
      const nextData = new Map();
      nextData.set('a', 1);
  ...
  describe('weakmaps', () => {
    describe('when not supported by environment', () => {
      ...
      test('handles weakmaps being undefined', () => {
        const data = {};
        expect(() => recycleNodesInto(data, data)).not.toThrow();
      });
    });

    test('does not recycle weakmaps with unequal values', () => {
      const a = {};
    ...
    test('does not recycle weakmaps with equal values', () => {
      const a = {};
      const prevData = new WeakMap();
      prevData.set(a, 1);
      const nextData = new WeakMap();
