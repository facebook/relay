...
class RelayFragmentResolver {
  ...
  _observe(): void {
    var dataID = this._fragmentPointer.getDataID();
    var prevChangeSubscription = this._changeSubscription;
    var prevData = this._data;
    var prevIDs = this._dataIDs;
    var nextChangeSubscription;
    var {
      data: nextData,
      dataIDs: nextIDs,
    ...
    if (nextData !== prevData || !prevChangeSubscription) {
      ...
      nextChangeSubscription = GraphQLStoreChangeEmitter.addListenerForIDs(
        Object.keys(nextIDs),
        () => this._onUpdate()
      );
    } else {
      nextChangeSubscription = prevChangeSubscription;
    }
  ...
  _updateGarbageCollectorSubscriptionCount(
    ...
    var maybeGarbageCollector =
      this._storeData && this._storeData.getGarbageCollector();
    if (maybeGarbageCollector) {
      var garbageCollector = maybeGarbageCollector;
