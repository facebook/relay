...
function runTest(setFlag, label) {
  describe('FragmentResource' + label, () => {
    let environment;
    let query;
...
runTest(
  flags => flags.ENABLE_FRAGMENT_RESOURCE_OPTIMIZATION === 'disabled',
  '',
);
runTest(
  flags =>
    flags.ENABLE_FRAGMENT_RESOURCE_OPTIMIZATION === 'reduce-snapshot-size',
  ':Remove seenRecords',
);
runTest(
  flags => flags.ENABLE_FRAGMENT_RESOURCE_OPTIMIZATION === 'epoch',
  ':Attach Epoch',
);
