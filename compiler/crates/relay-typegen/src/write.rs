...
pub(crate) fn write_operation_type_exports_section(
    ...
    if typegen_options.project_config.typegen_config.language == TypegenLanguage::TypeScript
...
pub(crate) fn write_fragment_type_exports_section(
    ...
    if let Some(refetchable_metadata) = refetchable_metadata {
        ...
        match typegen_options.project_config.js_module_format {
...
fn write_enum_definitions(
    ...
    for enum_id in enum_ids {
        ...
        if let Some(enum_module_suffix) = &typegen_options
            .project_config
            .typegen_config
            .enum_module_suffix
        {
...
pub(crate) fn write_validator_function(
    ...
    if fragment_definition.type_condition.is_abstract_type() {
        write_abstract_validator_function(
            typegen_options.project_config.typegen_config.language,
            fragment_definition,
            writer,
        )
    } else {
...
fn write_concrete_validator_function(
    ...
    let (open_comment, close_comment) = match typegen_options.project_config.typegen_config.language
    {
        TypegenLanguage::Flow | TypegenLanguage::JavaScript => ("/*", "*/"),
        TypegenLanguage::TypeScript => ("", ""),
    };
